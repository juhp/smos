...
consolidateMetaMapWithFiles clientMetaDataMap contentsMap =
  ...
  let go1 ::
      ...
      go1 s rf sfm@SyncFileMeta {..} =
        case lookupDirForest rf contentsMap of
          Nothing ->
      ...
      syncedChangedAndDeleted =
        M.foldlWithKey go1 Mergeful.initialClientStore $ dirForestToMap $ metaMapFiles clientMetaDataMap
      go2 ::
        Mergeful.ClientStore FileUUID SyncFile ->
        Path Rel File ->
        ByteString ->
   ...
   in M.foldlWithKey
        go2
        syncedChangedAndDeleted
        (dirForestToMap $ contentsMap `differenceDirForest` metaMapFiles clientMetaDataMap)
...
saveSyncFiles igf dir store = do
  cm <- case makeContentsMap store of
    Left _ -> die "This store does not represent a correct contents map." -- TODO this should be fixed after the new dirtree syncing
    Right cm -> pure cm
  saveContentsMap igf dir cm
